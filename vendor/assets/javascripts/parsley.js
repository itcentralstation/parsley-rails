/*!
* Parsley.js
* Version 2.2.0-rc4 - built Thu, Dec 10th 2015, 11:22 am
* http://parsleyjs.org
...
(function (global, factory) {
  ...
  ParsleyUI.prototype = {
    ...
    setupForm: function setupForm(formInstance) {
      formInstance.$element.on('submit.Parsley', false, function (evt) {
    ...
    manageFailingFieldTrigger: function manageFailingFieldTrigger(fieldInstance) {
      ...
      if (fieldInstance.options.multiple) $('[' + fieldInstance.options.namespace + 'multiple="' + fieldInstance.options.multiple + '"]').each(function () {
        if (!new RegExp('change', 'i').test($(this).parsley().options.trigger || '')) return $(this).on('change.ParsleyFailedOnce', false, function () {
          fieldInstance.validate();
        });
      });

      // Select case
      if (fieldInstance.$element.is('select')) if (!new RegExp('change', 'i').test(fieldInstance.options.trigger || '')) return fieldInstance.$element.on('change.ParsleyFailedOnce', false, function () {
        fieldInstance.validate();
      });

      // All other inputs fields
      if (!new RegExp('keyup', 'i').test(fieldInstance.options.trigger || '')) return fieldInstance.$element.on('keyup.ParsleyFailedOnce', false, function () {
        fieldInstance.validate();
      });
    },
  ...
  ParsleyForm.prototype = {
    ...
    whenValidate: function whenValidate(group, force, event) {
      ...
      var promises = this._withoutReactualizingFormOptions(function () {
        return $.map(_this3.fields, function (field) {
          ...
          if (!group || _this3._isFieldInGroup(field, group)) return field.whenValidate(force);
        });
      });

      var promiseBasedOnValidationResult = function promiseBasedOnValidationResult() {
        var r = $.Deferred();
        if (false === _this3.validationResult) r.reject();
        return r.resolve().promise();
      };

      return $.when.apply($, _toConsumableArray(promises)).done(function () {
        _this3._trigger('success');
      }).fail(function () {
        _this3.validationResult = false;_this3._trigger('error');
      }).always(function () {
        _this3._trigger('validated');
    ...
    whenValid: function whenValid(group, force) {
      ...
      var promises = this._withoutReactualizingFormOptions(function () {
        return $.map(_this4.fields, function (field) {
          // do not validate a field if not the same as given validation group
          if (!group || _this4._isFieldInGroup(field, group)) return field.whenValid(force);
        });
      });
    ...
    _bindFields: function _bindFields() {
      ...
      this._withoutReactualizingFormOptions(function () {
        _this5.$element.find(_this5.options.inputs).not(_this5.options.excluded).each(function (_, element) {
          var fieldInstance = new window.Parsley.Factory(element, {}, _this5);

          // Only add valid and not excluded `ParsleyField` and `ParsleyFieldMultiple` children
          if (('ParsleyField' === fieldInstance.__class__ || 'ParsleyFieldMultiple' === fieldInstance.__class__) && true !== fieldInstance.options.excluded) if ('undefined' === typeof _this5.fieldsMappedById[fieldInstance.__class__ + '-' + fieldInstance.__id__]) {
            _this5.fieldsMappedById[fieldInstance.__class__ + '-' + fieldInstance.__id__] = fieldInstance;
            _this5.fields.push(fieldInstance);
          }
        });

        $(oldFields).not(_this5.fields).each(function (_, field) {
  ...
  ConstraintFactory.prototype = {
    ...
    _parseRequirements: function _parseRequirements(options) {
      ...
      this.requirementList = this.validator.parseRequirements(this.requirements, function (key) {
        return options[_this6.name + capitalize(key)];
      });
    }
  };
  ...
  ParsleyField.prototype = {
    ...
    whenValidate: function whenValidate(force) {
      ...
      return this.whenValid(force, this.value).done(function () {
        _this7._trigger('success');
      }).fail(function () {
        _this7._trigger('error');
      }).always(function () {
        _this7._trigger('validated');
      });
    },
    ...
    whenValid: function whenValid(force, value) {
      ...
      $.each(groupedConstraints, function (_, constraints) {
        ...
        var promise = $.when.apply($, _toConsumableArray($.map(constraints, function (constraint) {
          return _this8._validateConstraint(value, constraint);
        })));
    ...
    _validateConstraint: function _validateConstraint(value, constraint) {
      ...
      return $.when(result).fail(function (errorMessage) {
        if (true === _this9.validationResult) _this9.validationResult = [];
        _this9.validationResult.push({
          assert: constraint,
    ...
    handleMultiple: function handleMultiple() {
      ...
      if ('undefined' !== typeof name) {
        $('input[name="' + name + '"]').each(function (i, input) {
          if ($(input).is('input[type=radio], input[type=checkbox]')) $(input).attr(_this10.options.namespace + 'multiple', _this10.options.multiple);
        });
      }
